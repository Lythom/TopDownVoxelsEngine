    ...
    public class MoveAnimCharacter : MonoBehaviour {
        ...
        private Vector3 HandleMove() {
            ...
            if (_controls.Gameplay.Jump.WasPressedThisFrame() && _jumpCooldown.TryPerform() && _position.y <= 0) {
                ...
                movement.y += JumpChargeIntensity * Time.fixedDeltaTime * (1 - Mathf.Clamp01(jumpCharge * jumpCharge));
            }

            _position += movement;
            transform.position = UnityEngine.Vector3.Lerp(transform.position, _position, VisualSnappingStrength * 50 * Time.deltaTime);
            transform.rotation = Quaternion.Slerp(transform.rotation, _rotation, VisualSnappingStrength * 50 * Time.deltaTime);
            ...
            // Rotation
            //if (_controls.Gameplay.RotateCamera.IsPressed()) {
            // Zooming
