...
// imported code, do not check
// ReSharper disable All

// From https://forum.unity.com/threads/text-mesh-pro-does-not-work-with-spriteatlas-assets.697088/#post-7581571
public class SpriteAtlasConverter {
    ...
    private static void RepackAtlas(SpriteAtlas atlas) {
        ...
        var pngPath = path.Replace(".spriteatlasv2", ".png");
        ...
        if (ti == null) throw new Exception($"Asset at {pngPath} must be imported with TextureImporter. Currently: {AssetImporter.GetAtPath(pngPath).GetType().Name}");
        ti.textureType = TextureImporterType.Sprite;
        ti.spriteImportMode = SpriteImportMode.Multiple;
        ti.spritesheet = metas.ToArray();
        var ts = ti.GetDefaultPlatformTextureSettings();
        ts.maxTextureSize = 2048;
